...
{
	//TODO: do it yourself
	if((glm::length(pos2 - pos1) <= (r1 + r2) * (r1 + r2)))
	{
		return true;
	}
	return false;
...
bool OverlapAABB2AABB(const glm::vec3& min1, const glm::vec3& max1, const glm::vec3& min2, const glm::vec3& max2)
{
	//TODO: do it yourself
	if (max1.x < min2.x or
		min1.x > max2.x or
	...
	const glm::vec3& lineStart, const glm::vec3& lineEnd)
{
	...
	if (lineTangent.x == 0.0f && lineTangent.y == 0.0f && lineTangent.z == 0.0f) return false;  // Not a line, just a point.

	float lineLength = glm::length(lineLength);
	/*lineTangent.Normalize();*/
	glm::normalize(lineTangent);

	// Step 1: Calculate normal to the line (CCW 90-deg rotation)
	glm::vec3 lineNormal = glm::vec3(-lineTangent.y, lineTangent.x, 0);  // Assuming 2D vectors

	// Step 2: Project circle position onto the line's normal to find shortest distance
	glm::vec3 toCircle = circlePos - lineStart;
	//float distanceToLine = abs(toCircle.Dot(lineNormal));

	float distanceToLine = abs(glm::dot(toCircle, lineNormal));
	...
	float projectedDist = glm::dot(toCircle, lineTangent);
...
void ResolveCircle2StaticLine(PhysicsObject& ball, float radius, const glm::vec3& lineStart, const glm::vec3& lineEnd)
{
	...
	glm::vec3 toCircle = ball.pos - lineStart;
	...
	glm::vec3 collisionVec = ball.pos - closestPoint;
	...
	if (distance < radius)
	{
		...
		// Resolve penetration
		glm::vec3 normal = collisionVec * (1 / distance); // Normalize collision vector
		ball.pos += normal * penetrationDist;

		// Resolve velocity
		//ball.vel -= ball.vel.Dot(normal) * normal; // Reflect velocity along the normal

		glm::vec3 reflected_vel = ball.vel - 2.0f * glm::dot(ball.vel, normal) * normal;

		// Update the ball's velocity
		ball.vel = reflected_vel;
	}
}
...
bool OverlapAABB2Line(const glm::vec3& boxMin, const glm::vec3& boxMax, const glm::vec3& lineStart, const glm::vec3& lineEnd)
{
	glm::vec3 lineTangent = lineEnd - lineStart;
	if (lineTangent.x == 0.0f && lineTangent.y == 0.0f && lineTangent.z == 0.0f) return false;  // Not a line, just a point.
...
bool OverlapSphere2AABB(PhysicsObject& obj1, float r, PhysicsObject& obj2, glm::vec3 Min, glm::vec3 Max, CollisionData& cd)
{
	...
	if (isInsideAABB) {
		...
		if (minPen == penX) {
			cd.normal = (dxLeft < dxRight) ? glm::vec3(-1, 0, 0) : glm::vec3(1, 0, 0);
		}
		else if (minPen == penY) {
			cd.normal = (dyBottom < dyTop) ? glm::vec3(0, -1, 0) : glm::vec3(0, 1, 0);
		}
		else { // minPen == penZ
			cd.normal = (dzFront < dzBack) ? glm::vec3(0, 0, -1) : glm::vec3(0, 0, 1);
		}
	}
	else {
		// Sphere is outside AABB, check distance to closest point
		diff = obj1.pos - closestPoint;
		distanceSq = glm::length2(diff);
...
void ResolveCollision(CollisionData& cd) 
{
	if(cd.pObj2 == nullptr)
	{
		...
		float velocityDot = glm::dot(circle.vel, cd.normal);
	...
	else if(cd.pObj2 != nullptr)
	{
		...
		if (obj1.mass > 0 && obj2.mass > 0)
		{
			...
			// Resolve Velocity
		/*	glm::vec3 deltaVel = (obj1.vel.Dot(cd.normal) - obj2.vel.Dot(cd.normal)) * cd.normal;*/
			glm::vec3 deltaVel = (glm::dot(obj1.vel, cd.normal) - glm::dot(obj2.vel, cd.normal)) * cd.normal;
		...
		else
		{
			...
			float dot = glm::dot(obj1.vel, cd.normal);
...
bool OverlapAABB2AABB(PhysicsObject& box1, glm::vec3 box1min, glm::vec3 box1max, PhysicsObject& box2, glm::vec3 box2min, glm::vec3 box2max, CollisionData& cd)
{
	...
	if (minPen == overlapX)
		cd.normal = (box1.pos.x < box2.pos.x) ? glm::vec3(-1, 0, 0) : glm::vec3(1, 0, 0);
	else if (minPen == overlapY)
		cd.normal = (box1.pos.y < box2.pos.y) ? glm::vec3(0, -1, 0) : glm::vec3(0, 1, 0);
	else
		cd.normal = (box1.pos.z < box2.pos.z) ? glm::vec3(0, 0, -1) : glm::vec3(0, 0, 1);
...
bool OverlapSphere2Line(PhysicsObject& obj1, float radius, const glm::vec3& lineA, const glm::vec3& lineB, CollisionData& cd)
{
	// Get line segment vectors
	glm::vec3 lineDir = lineB - lineA;
	glm::vec3 toCircle = obj1.pos - lineA;

	// Project circle's position onto the line segment
	/*float t = toCircle.Dot(lineDir) / lineDir.LengthSquared();*/
	float t = glm::dot(toCircle, lineDir) / glm::length2(lineDir);
	t = Math::Clamp(t, 0.0f, 1.0f);
	glm::vec3 closestPoint = lineA + lineDir * t;

	// Calculate distance from circle to closest point
	glm::vec3 diff = obj1.pos - closestPoint;
